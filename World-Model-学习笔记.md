...
1. [**World Models**](https://arxiv.org/abs/1803.10122) (Ha & Schmidhuber, 2018) — 经典起点,VAE + MDN-RNN + 小 controller([交互式网站](https://worldmodels.github.io/))
   ...
   **小贴士**:每代只在 C 更新后重新算一次 B 和 D(`eigendecompose(C)`),然后这一代的 λ 次采样全部复用,计算上很划算。
   ...
   > ⚠️ **先澄清一个常见混淆**:本小节讲的是「**如何更新 C**」,与前面「`C = B·D²·Bᵀ` 特征分解」是**两个不同的步骤**:
   > - **采样阶段**(每代步骤 ①②):从 C 中提取 B、D,用 `x = m + σ·B·D·z` 生成候选
   > - **更新阶段**(每代步骤 ⑤,即本节):用本代的反馈信息**修改 C 本身**
   > - 两步前后衔接:先用旧 C 的 B、D 采样 → 评估 → 再更新 C → 下一代再分解新 C

   CMA-ES 不是凭空更新 C,而是收集**两种独立证据**,合并后小幅度调整 C。

   ##### 信号 1:Rank-μ Update —— "本代选优集中在哪个方向?"

   **直觉**:观察本代选出的 μ 个最优候选,它们集中在哪个方向?C 就在那个方向加粗。
   ...
   **直觉**:不只看本代,还要看**历代均值移动的累积方向**(进化路径 $p_c$)。如果连续几代都往同一方向走,这就是个强信号 → 沿这个长期方向加强 C。
   ...
   **数学** —— 先维护进化路径(指数滑动平均):
   ...
   <p align="center">
     ...
     <i>① 左:Rank-μ 用 top-μ 候选(黄圈)的样本协方差构造一个椭圆(蓝色),把 C 朝那个集中方向加粗;② 中:Rank-1 累积历代均值 m₀ → m₅ 的移动方向得到进化路径 p_c(红色粗箭头),其外积给出沿该方向的"杆状"贡献;③ 右:新 C(粗绿)= 旧 C(虚线) + Rank-μ 蓝色蓝点 + Rank-1 红色虚线,三者加权融合。</i>
   </p>
   ...
   - 第一项 $(1 - c_1 - c_\mu) C$:**保留旧 C 的大部分**(防止剧烈震荡)
   ...
   <p align="center">
     ...
     <i>每代完整流程:① 特征分解 C → 得到 B, D ② 用 B, D 采样 λ 个候选 ③ 评估并排序 ④ 更新均值 m 和进化路径 p_c ⑤ 用 Rank-μ + Rank-1 更新 C ⑥ 更新全局步长 σ。下一代回到 ① 用新 C 再分解。</i>
   </p>

   **关键洞察**:**「采样的 B, D」和「更新的 C」是同一个矩阵的两面**——更新阶段调整 C 本身,下一代的采样阶段再从新 C 提取 B、D 给候选生成用。`C = B·D²·Bᵀ` 是"翻译工具",`C ← ...` 才是"学习行为"。
