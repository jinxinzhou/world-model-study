...
| 角度 | 解释 |
|---|---|
| **设计意图** | RSSM 设计 $s_t$ 为"对历史的压缩表示";训练成功时 $s_{t-1}$ 既包含 $s_{<t-1}$ 也包含 $o_{1:t-1}, a_{1:t-2}$ 的全部相关信息 |
| **模型对称性** | 生成模型本身是 Markov 的(transition 只依赖 $s_{t-1}, a_{t-1}$);让 $q$ 也保持同样的 Markov 结构是最自然的选择 |
| **工程必要性** | 不这样做的话,encoder 每步要吃整段历史 $o_{1:t}$,时间和内存变 $O(T^2)$ |
| **隐式信息流** | 递归看:$s_{t-1}$ 由 $s_{t-2}$ + $o_{t-1}$ 算出,$s_{t-2}$ 又由 $s_{t-3}$ + $o_{t-2}$,...... 所以 $o_{<t}$ 的信息**通过 $s$ 链路全部"流"到了 $s_{t-1}$ 里** |
...
| Step B 之后(filtering,丢未来) | $s_{<t}, o_{1:t}, a_{1:t-1}$ |
| Step C 之后(Markov,丢远期过去)⭐ PlaNet 实际用的 | $s_{t-1}, a_{t-1}, o_t$ |
