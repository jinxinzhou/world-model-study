...
##### Step 1:Vanilla 隐变量序列模型

为了端到端地学一个能模拟 POMDP 的世界模型,最简单的形式就是一个**有隐变量的序列模型** —— 用 NN 把 §🧭 问题定义里的真实环境(直体 p)参数化成可学的世界模型(斜体 p_θ):

<p align="center"><img src="asset/formulas/f20.png" width="520"/></p>

**此时还没有 h,没有 RSSM**,只是一个最朴素的隐变量 SSM。
...
|---|---|---|
| 重建 ln p(o_t ∣ s_t) | decoder + encoder | "s_t 要够还原图像" |
| reward ln p(r_t ∣ s_t) | reward model + encoder | "s_t 要够预测奖励" |
| KL[q ∥ p] | transition + encoder | "s_t 要让下一步可预测" |
