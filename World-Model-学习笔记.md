...
**Step B — 马尔可夫近似(丢掉 $s$ 的远期历史)**

$$q(s_t \mid s_{<t}, \cdots) \;\approx\; q(s_t \mid s_{t-1}, \cdots)$$

**理由**:**生成模型本身是 Markov 的**(transition 只依赖 $s_{t-1}, a_{t-1}$)。既然 $p_\theta$ 是马尔可夫,让 $q$ 也保持同样的链式结构是**保留模型对称性**最自然的选择。

**Step C — filtering 近似(丢掉未来观测/动作)**
...
**Step D — encoder 的 Markov 充分性(把过去 obs/action 历史压进 $s_{t-1}$)**

$$q(s_t \mid s_{t-1}, o_{1:t}, a_{1:t-1}) \;\approx\; q(s_t \mid s_{t-1}, a_{t-1}, o_t)$$

**关键假设**:$s_{t-1}$ 已经是过去所有 obs/action 的**充分统计量**,所以 $o_{1:t-1}$ 和 $a_{1:t-2}$ 都可以从条件里去掉,**只留新增信息** $o_t$ 和 $a_{t-1}$。这是 PlaNet 做得**最大胆的一步**。

**为什么这一步合理?**

| 角度 | 解释 |
|---|---|
| **设计意图** | 整个 RSSM 就是要让 $s_t$ 成为"对历史的压缩表示";训练成功时 $s_{t-1}$ **应该**包含 $o_{1:t-1}, a_{1:t-2}$ 的全部相关信息 |
| **结构一致性** | 与生成模型的 Markov 性**对称** —— 生成时 $s_{t-1} \to s_t$ 不看 $s_{<t-1}$;推断时也 $s_{t-1} \to s_t$ 不看 $o_{<t}, a_{<t-1}$ |
| **工程必要性** | 不这样做的话,encoder 每步要吃整段历史 $o_{1:t}$,时间和内存变 $O(T^2)$ |
| **隐式信息流** | 递归地看:$s_{t-1}$ 是用 $s_{t-2}$ 和 $o_{t-1}$ 算出来的,$s_{t-2}$ 又用 $s_{t-3}$ 和 $o_{t-2}$,...... 所以 $o_{1:t-1}$ 的信息**通过 s 链路全部"流"到了 $s_{t-1}$ 里** |

> ⚠️ **这个假设有风险吗?** 有 —— 但 ELBO 优化本身会去修正它。如果 $s_{t-1}$ 不够充分(比如训练不够),近似就差、ELBO 松,KL 项会反向逼着 encoder 把更多"对未来有用的"信息塞进 $s_{t-1}$。这就是 **encoder + transition 必须联合训** 的根本原因。

> 💡 注意:虽然每个 $q$ 因子的**直接条件**只是 $(s_{t-1}, a_{t-1}, o_t)$,但因为 $s_{t-1}$ 本身是从上一步的 $q$ 采样出来的、那个 $q$ 又依赖 $s_{t-2}$ 和 $o_{t-1}$...... 所以**整条轨迹的 $q$ 实际上隐式依赖了 $o_{\le t}$**。chain rule + 顺序采样自动把"历史观测"通过 $s$ 传下来了。

**四步合起来 —— 条件集合的演化**

| 阶段 | 条件集合 |
|---|---|
| **真后验全展开**(Step A) | $s_{<t}, o_{1:T}, a_{1:T}$ |
| Step B 之后(Markov on s) | $s_{t-1}, o_{1:T}, a_{1:T}$ |
| Step C 之后(filtering) | $s_{t-1}, o_{1:t}, a_{1:t-1}$ |
| Step D 之后(encoder Markov)⭐ PlaNet 实际用的 | $s_{t-1}, a_{t-1}, o_t$ |

下面再从四个维度补充说明。

**(1) q 在近似什么?**

$q$ 近似的是**在观察到完整观测序列和动作序列之后,对整条隐状态轨迹的概率分布** —— 即真实后验 $p_\theta(s_{1:T} \mid o_{1:T}, a_{1:T})$。

| | 真实后验 $p_\theta$ | 近似后验 $q$ |
|---|---|---|
| 属于谁 | 模型本身的内禀分布 | 额外加的一个 NN(encoder) |
| 能不能算 | 理论存在,但**算不出来** | 设计成能算的形式 |
| 目标 | — | 通过优化让 $q$ 尽量接近 $p_\theta$ |
...
|---|---|---|
| 重建 ln $p(o_t \mid s_t)$ | decoder + encoder | "$s_t$ 要够还原图像" |
| reward ln $p(r_t \mid s_t)$ | reward model + encoder | "$s_t$ 要够预测奖励" |
...
这就是 **RSSM** 名字的由来:把 **R**ecurrent NN(确定性)+ **S**tate-**S**pace **M**odel(随机)**缝合**起来。
