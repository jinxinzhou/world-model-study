...
| 方案 | 名字 | 优劣 |
|---|---|---|
| 只看 $o_{\le t}$ | **filtering posterior** | 推理时(部署/规划)只能看到过去,本来就**没有未来 $o$** |
| 看完整 $o_{1:T}$ | **smoothing posterior** | 训练时是有完整轨迹的,能用更多信息估计 $s_t$ |

PlaNet 选 filtering,核心原因:**训练时学的 $q$ 必须和部署时用的 $q$ 是同一个**。如果训练时让 $q$ 偷看未来观测,部署时根本拿不到那些信息,$q$ 就用不了。
