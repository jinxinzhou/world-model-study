...
<details>
<summary>📖 <b>§3 论文阅读地图:怎么去理解第三章</b>(点开展开)</summary>

§3 第一遍读上去公式很多很乱,其实它是一个**4 步递进的故事 —— 每一步都在解决上一步的痛点**。按这个脉络读,会发现它是结构非常工整的"小论文"。

##### 脉络一览

```
Step 1: 朴素的 latent SSM            [Eq. 2]
   ↓ "后验算不出来,怎么训?"
Step 2: 引入 variational encoder
   ↓ "训练目标是什么?"
Step 3: 推导 ELBO                     [Eq. 3]
   ↓ "纯随机隐变量记不住信息,怎么办?"
Step 4: 加 deterministic path → RSSM  [Eq. 4]
```

##### Step 1 — 最朴素的 latent state-space model(Eq. 2)

论文先写出最简单的"有隐变量的序列模型":Transition / Observation / Reward 三个高斯分布,NN 参数化。**此时还没有 h,没有 RSSM**。

> 类比 VAE 论文先讲"latent + Gaussian decoder",再说怎么训。先把骨架立起来。

##### Step 2 — Variational encoder("后验算不出来怎么办")

**问题**:训练 Step 1 那个模型理论上应该从真后验 p(s_t ∣ o_{≤t}, a_{<t}) 采样,但这个后验**算不出来**(NN 非线性,没法解析积分)。

**论文的解**:引入近似后验 q,也用 NN 参数化:

q(s_{1:T} ∣ o_{1:T}, a_{1:T}) = ∏_t q(s_t ∣ s_{t-1}, a_{t-1}, o_t)

注意三件事:
1. 这就是 VAE 里的 **encoder**,但是**轨迹版**
2. 它是 **filtering 后验**(只看 o_{≤t},不看未来),因为部署时也只能看到过去
3. PlaNet 用的是 **mean-field**:q 被分解成每一步独立的 q(s_t ∣ ...)

> 🔑 所有变分推断方法都要走这一步 —— "真后验不行,用近似的"。

##### Step 3 — 推导 ELBO(Eq. 3)

**问题**:有了 encoder,怎么用它训练模型?

**论文的解**:用 Jensen 不等式得到对数似然的下界(ELBO):

<p align="center"><img src="asset/formulas/f14.png" alt="PlaNet ELBO" width="780"/></p>

读这个公式的**正确姿势**:
- **第一项(重建)**:从 encoder 采出的 s_t 能不能重建出真实的 o_t? → 训练 **decoder**
- **第二项(KL)**:encoder 推出的后验,能不能和"用 transition 模型从 s_{t-1} 滚出来"的先验对齐? → 训练 **transition + encoder**
- **整个 ELBO**:把"重建 obs"和"动力学一致"两件事**统一在一个目标里**

> 🔑 **KL 项是 §3 的灵魂**。它强迫 encoder 不能乱编 s_t —— 必须编成"动力学认可的形式"。这是 PlaNet 端到端训练的关键。

> ⚠️ 论文为简化只列了 observation 部分,**reward 项完全对称**:把 ln p(o_t ∣ s_t) 换成 ln p(r_t ∣ s_t) 加进去即可。

##### Step 4 — 加 deterministic path → RSSM(Eq. 4)

**问题**:Step 3 那个模型,所有信息靠**随机的 s_t** 在时间上传递。但 s_t 每步要采样 —— **采样引入噪声**,长程信息很快被冲掉,模型记不住"5 步之前发生了什么"。

**论文的解**:加一条**纯确定性的并行通路** h_t,专门负责记忆:
...
| 部分 | 作用 | 类比 |
|---|---|---|
| **h_t**(确定性) | 长程记忆、信息无损传递 | RNN 的 hidden state |
| **s_t**(随机) | 表达环境不确定性、未来多种可能 | VAE 的 latent |

这就是 **RSSM** 名字由来:把 **R**ecurrent NN(确定性)+ **S**tate-**S**pace **M**odel(随机)**缝合**起来。

> 🔑 论文 Figure 2 一图说尽:**(a) 纯确定性 RNN** = 能记不能表达不确定性 | **(b) 纯随机 SSM** = 能表达不确定性记不住 | **(c) RSSM = a + b** = 两全其美

##### 把 §3 串成一句话

> **§3 = "我要一个能训、能记、能采样的隐空间动力学模型"**
> - 能训 → 需要 encoder(Step 2)+ ELBO(Step 3)
> - 能记 → 需要 deterministic h(Step 4)
> - 能采样 → 需要 stochastic s(Step 1, 4)
...
| "Eq.3 里的期望怎么计算?" | 重参数化采样一个 s_{t-1},再代入。**单样本估计 + reparameterization** 就够了。 |
| "为什么不直接用 RNN 当 transition?" | Step 4 解释了:RNN 确定性,**不能表达"我不知道下一步是什么"**。在 model-based RL 里,让 planner 知道模型自己有多不确定**至关重要**。 |

##### 建议读 §3 的顺序

1. **先看 Figure 2 (a/b/c)** —— 5 分钟掌握 4 步演化的视觉直觉
2. **读 Eq. 2** —— 朴素 SSM,5 分钟
3. **跳到 Eq. 4 看 RSSM 最终形态** —— 知道目的地长什么样
4. **回头读 Eq. 3 的 ELBO 推导** —— 此时已经知道 prior 长什么样,KL 项就好理解了
5. **最后读 "Deterministic path" 那段动机** —— 明白为什么不能只用随机或只用确定性

按论文从上往下读会很卡,因为 Eq. 3 写完后才说"这模型其实不太行"。**先看到 (c) 这个目的地,再回头补推导,效率高得多**。
