...
| World Models Pain Point / Limitation | PlaNet Response | See chapter |
|---|---|---|
...
#### 3. The real value of explicit formalization (not just terminology)
...
> 💡 $D$ = the **maximum prediction horizon** of latent overshooting (the $D$ in `d = 1, ..., D` from the pseudocode above) — training forces the model to predict accurately at 1, 2, ..., $D$ steps ahead. $D = 1$ reduces to the original §🧬 RSSM ELBO; PlaNet uses $D = 50$ in practice, roughly the training sequence length, so basically "every possible horizon during training is covered". Carrying $D = 50$ is only possible because the Latent Overshooting fix below replaces the per-horizon cost "decoder forward" with "transition forward + closed-form Gaussian KL".
>
> ⚠️ **Do not confuse this with the CEM planning horizon $H = 12$**: $D$ is the maximum span of the multi-step KL **at training time** (offline regularization strength); $H$ is the number of steps CEM rolls forward **at deployment** (online planning depth). $D > H$ is a sensible design — train it for longer than you plan, so inference can plan further without drift.
...
1. **Neither encoder nor decoder gets multiplied by $D$**
   - **Encoder isn't multiplied**: in the KL sum $\sum_{d=1}^{D} \mathrm{KL}[q_\phi(s_t \mid h_t, o_t) \,\|\, p_\theta^{(d)}(s_t \mid s_{t-d}, h_{t-d}, a_{t-d:t-1})]$, **only the prior side depends on $d$ — the posterior side does not** → the same $q_\phi(s_t \mid h_t, o_t)$ is logically reused inside all $D$ KL terms, so encoder is called $T$ times total, not $T \times D$
...
#### 1. System data flow (bird's-eye view) ↔ paper Algorithm 1 side-by-side

The PlaNet system consists of two loops that drive each other — **training** (real env → Buffer → RSSM) and **deployment** (RSSM → CEM → real env) — linked by a single replay buffer. Below the **bird's-eye data flow** (left) is placed next to the **paper Algorithm 1** (right) for direct comparison:
...
flowchart TD
    ...
    subgraph A_phase [" "]
        ...
        Train["Training step<br/>§🔭 ELBO (line 6)<br/>θ ← θ − α∇L (line 7)"]
...
PlaNet training is **not "train once on a fixed dataset"** — the data is generated by the agent itself, so like any RL setting it must **alternate model updates and data collection** (the two inner `for` loops on the right of #### 1's Algorithm 1):
...
- **Stage B (lines 8-16)** uses the improved model + CEM to roll one new episode — **data improves** (covers more "interesting" states); line 10's "infer belief from history" is actually the §🧬 RSSM dual path: first $h_t = f_\mathrm{GRU}(h_{t-1}, s_{t-1}, a_{t-1})$, then $s_t \sim q_\phi(s_t \mid h_t, o_t)$
- The two loops feed each other; model and data **co-iterate to convergence**

Key hyperparameters (paper's DMC defaults):

| Hyperparam | Meaning | Typical value |
|---|---|---|
| $S$ | seed episodes (initial random data) | 5 |
| $C$ | model updates per outer iteration | 100 |
| $B$ | training batch size | 50 |
| $L$ | training chunk length | 50 |
| $R$ | action repeat (how many times to repeat each action) | 2 ~ 4 (task-dependent) |
...
##### 🖼️ Visual intuition: how does CEM converge?
...
##### Algorithm implementation: plan_action

```python
def plan_action(world_model, current_state):
    # Maintain an action-sequence distribution: independent Gaussian per step
    μ = zeros(H, action_dim)        # H = 12 step planning horizon
    σ = ones(H, action_dim)
    ...
    for iteration in range(I):       # I = 10 CEM iterations
        ...
        # 2. Roll out each sequence in the world model and accumulate reward
        returns = []
        for seq in action_seqs:
            h, s = current_state
            R = 0
            for t in range(H):
                h, s = world_model.transition_step(h, s, seq[t])  # use prior, no obs
                R += world_model.reward(h, s)
            returns.append(R)

        # 3. Pick top-K (K = 100) elites
        elite_idx = argsort(returns)[-K:]
        elite_seqs = action_seqs[elite_idx]

        # 4. Update the distribution to the elites' mean and variance
