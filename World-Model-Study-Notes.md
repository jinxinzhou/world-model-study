...
PlaNet = **4 networks** in 1:1 correspondence with the POMDP's four pieces (full comparison in §🧭 #### 2):

| POMDP role | PlaNet network | Form |
|---|---|---|
...
```python
# PlaNet training outer loop (paraphrased from paper Algorithm 1)

# ---- initialization ----
D = empty_replay_buffer
fill D with S random-action seed episodes    # S ≈ 5
θ = random_init                              # params of 4 networks + GRU

while not converged:
    # ===== (A) Model fitting: train the model C times on current buffer =====
    for step in 1..C:                        # C ≈ 100
        # Sample B chunks of length L from the buffer uniformly at random
        batch = sample_chunks(D, batch_size=B, chunk_len=L)   # B ≈ 50, L ≈ 50
        # Compute the Latent Overshooting ELBO loss from §🔭
        loss = ELBO_with_overshooting(batch, θ)
        # SGD update
        θ ← θ − α · ∇_θ loss

    # ===== (B) Data collection: run one new episode with the current model =====
    o = env.reset()
    h, s = init_h, init_s
    a_prev = zeros(action_dim)
    for t in 1..⌈T / R⌉:                    # R = action repeat
        # 1) encode the current obs into a belief
        h = GRU(h, s, a_prev)
        ...
        # 3) Action repeat: execute the same a in the real env R times, sum the reward
        r_total = 0
        for k in 1..R:
            r, o_next = env.step(a)
            r_total += r
...
- **Stage A** trains the model $C$ more steps on the current buffer — **model improves**
- **Stage B** uses the improved model + CEM to roll one new episode — **data improves** (covers more "interesting" states)
